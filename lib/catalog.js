...
let blobSignerLoader = () => import("@vercel/blob");
...
export async function materializeArtifact(artifact, blobSigner) {
  let url = artifact.url;
  if (!url) {
    const { issueSignedToken, presignUrl } = blobSigner ?? await blobSignerLoader();
...
export function setBlobSignerLoaderForTests(loader) {
  blobSignerLoader = loader;
}
